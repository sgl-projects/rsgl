...
test_that("ggplot has correct mapping for cta-generated columns", {
  ...
  expect_equal(ggplot2::as_label(layer_mapping$x), "rsgl.linear.bin.30.hp")
  expect_equal(ggplot2::as_label(layer_mapping$y), "rsgl.log.bin.5.mpg")
...
test_that(
  paste(
    "ggplot layer doesn't have orientation",
    "if geom doesn't have direction"
  ),
  {
    rgs <- sgl_to_rgs("
			visualize
    ...
    expect_false("orientation" %in% p$layers[[1]]$geom_params)
  }
)

test_that("ggplot layer has orientation if geom has direction", {
  rgs <- sgl_to_rgs("
    visualize
      bin(mpg) as y,
			count(*) as x
  ...
  expect_equal(p$layers[[1]]$geom_params$orientation, "y")
})
