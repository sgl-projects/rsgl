...
test_that("sgl_func_name returns ln", {
  expect_equal(sgl_func_name(test_ln), "ln")
})
...
describe("valid_scale", {
  ...
  describe("non-positive values", {
    ...
    describe("layer has non-pos value", {
      it("raises error", {
        expected_msg <- paste(
          "Error: the ln scale can only be applied to aesthetics",
          "where all values from mappings are positive."
        )
        expect_error(
          valid_scale(ln, "y", layers, dfs),
          expected_msg,
          fixed = TRUE
        )
      })
