...
test_that("add_transformed_column doesn't modify column if already exists", {
  ...
  df <- data.frame(rsgl.linear.bin.col_1 = existing_col_values)
  bin_cta <- new_sgl_cta_bin()
  scale <- new_sgl_scale_linear()
